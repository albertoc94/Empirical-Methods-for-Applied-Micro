...
ggplot(lasso_df,aes(x=lambda,y=cv_mse)) + geom_line(size=1) + geom_vline(xintercept = best_lam, linetype="dotted", 
                color = "red", size=1.5)
...
df = as.data.frame(cbind(x_test,y_test,pred))
colnames(df) = c(x_col,"y_test","pred")
ggplot(df,aes(x=y_test,y=pred)) + geom_point(size=1) + xlab("actual house price") + ylab("predicted house price")
```
...
ggplot(ridge_df,aes(x=lambda,y=cv_mse)) + geom_line(size=1) + geom_vline(xintercept = best_lam, linetype="dotted", 
                color = "red", size=1.5)
```

```{r, echo=FALSE}
ridge_best <- glmnet(x_train, y_train, alpha = 0, lambda = best_lam)
pred <- predict(ridge_best, s = best_lam, newx = x_test)
df = as.data.frame(cbind(x_test,y_test,pred))
colnames(df) = c(x_col,"y_test","pred")
ggplot(df,aes(x=y_test,y=pred)) + geom_point(size=1) + xlab("actual house price") + ylab("predicted house price")
```
